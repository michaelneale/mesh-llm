...
bundle output="/tmp/mesh-bundle.tar.gz":
    ...
    cp {{ build_dir }}/bin/llama-moe-components "$BUNDLE/"
    for lib in {{ build_dir }}/bin/*.dylib; do
        cp "$lib" "$BUNDLE/" 2>/dev/null || true
    done
    # Fix rpaths for portability
    for bin in "$BUNDLE/mesh-llm" "$BUNDLE/$rpc_name" "$BUNDLE/$llama_name" "$BUNDLE/llama-moe-analyze" "$BUNDLE/llama-moe-split" "$BUNDLE/llama-moe-components"; do
        [ -f "$bin" ] || continue
