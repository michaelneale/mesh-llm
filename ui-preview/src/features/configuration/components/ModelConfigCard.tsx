...
import { contextGB, contextGBPerK, findModel, kvGB, modelFamilyColorKey } from '@/features/configuration/lib/config-math'
import type { ConfigAssign, ConfigModel, ConfigNode } from '@/features/app-tabs/types'

type ModelConfigCardProps = { assign: ConfigAssign; node: ConfigNode; models?: ConfigModel[]; containerFreeGB: number; controlTabIndex?: number; onCtxChange: (ctx: number) => void; onRemove: () => void }

function formatGB(value: number): string { return `${value.toFixed(1)} GB` }
...
function formatOptional(value: number | string | undefined): string { return value === undefined ? 'auto' : `${value}` }

export function ModelConfigCard({ assign, node, models, containerFreeGB, controlTabIndex, onCtxChange, onRemove }: ModelConfigCardProps) {
  ...
  const maxAllowedCtx = Math.max(CTX_MIN, containerFreeGB > 0 ? (containerFreeGB / contextGBPerK(model)) * 1024 : CTX_MIN)
  const safeCtx = Math.min(CTX_MAX, maxAllowedCtx)
  const selectedGpu = node.gpus.find((gpu) => gpu.idx === assign.containerIdx)
  const locationLabel = node.placement === 'pooled' ? `${node.hostname} pool` : `GPU ${assign.containerIdx} · ${selectedGpu?.name ?? 'unknown'}`
  ...
  const errorText = modelShortfallGB > 0
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <span className="rounded-full border border-border-soft bg-background px-2 py-0.5 font-mono text-[length:var(--density-type-annotation)] text-fg-dim">{paramsLabel}</span>
        ...
        <button className="ui-control-destructive inline-flex size-[30px] shrink-0 items-center justify-center rounded-[var(--radius)] border" onClick={onRemove} tabIndex={controlTabIndex} title="Remove" type="button" aria-label={`Remove ${model.name} from ${locationLabel}`}>
          <Trash2 aria-hidden="true" className="size-[15px]" strokeWidth={1.8} />
        </button>
      </div>

      <div className="mt-3.5 flex flex-wrap gap-[5px]">
        {details.map(([label, value]) => <DetailPill key={label} label={label} value={value} />)}
      </div>
      ...
      {hasError ? (
        <div className="mt-2.5 rounded-[var(--radius)] border border-bad/70 bg-bad/10 px-2.5 py-2 text-[length:var(--density-type-caption)] font-medium text-bad">
          {errorText} Reduce context, remove another allocation, or move the model to a larger {node.placement === 'pooled' ? 'pool' : 'GPU'}.
        </div>
      ) : null}

      <div className="mt-2.5">
        <CtxSlider value={assign.ctx} onChange={onCtxChange} maxCtx={safeCtx} invalid={hasError} controlTabIndex={controlTabIndex} />
