...
import { BrainCircuit, ChevronDown, Cloud, Cpu, Eye, Network, PanelsTopLeft, SlidersHorizontal, Sparkles, Wind, type LucideIcon } from 'lucide-react'
...
type ModelCatalogProps = { models: ModelSummary[]; filterLabel: string; onFilterClick?: () => void; onSelect?: (model: ModelSummary) => void; selectedModelName?: string }
...
function getModelProvider(model: ModelSummary) {
  const candidates = [model.family, model.name]
    .map((value) => value.trim().toLowerCase())
    .filter(Boolean)

  return MODEL_PROVIDERS.find((provider) => provider.prefixes.some((prefix) => candidates.some((candidate) => modelMatchesProvider(candidate, prefix)))) ?? UNKNOWN_PROVIDER
}
...
function StatusBadge({ status }: { status: ModelSummary['status'] }) {
  ...
  const pillBg = status === 'ready' || status === 'warm'
  ...
  const pillBorder = status === 'ready' || status === 'warm'
...
function ModelRow({ model, active, onSelect }: { model: ModelSummary; active: boolean; onSelect?: (model: ModelSummary) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        {model.fullId && <div className="mt-0.5 truncate font-mono text-[length:var(--density-type-label)] text-fg-faint">{model.fullId}</div>}
        <div className="mt-1.5 flex items-center gap-1.5">
          ...
          <span className="font-mono text-[length:var(--density-type-label)] text-fg-faint">{model.nodeCount ?? 1} node</span>
          <span className="text-[length:var(--density-type-label)] text-fg-faint">·</span>
      ...
      <div className="flex flex-col items-end gap-[5px]">
        ...
        <span className="inline-flex items-center rounded-full border border-border px-[7px] py-px text-[length:var(--density-type-label)] font-medium text-fg-faint">{architectureLabel}</span>
      </div>
    </button>
  )
}
    ...
    <aside className="panel-shell overflow-hidden rounded-[var(--radius-lg)] border border-border bg-panel">
      <header className="flex items-center justify-between border-b border-border-soft px-3.5 py-2.5">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onFilterClick} type="button" className="ui-control-ghost inline-flex items-center gap-1.5 rounded-[var(--radius)] px-1.5 py-1 text-[length:var(--density-type-caption)]">
      ...
      <div className="overflow-y-auto" style={{ maxHeight: 492 }}>
        {models.map((model) => <ModelRow key={model.name} active={model.name === selectedModelName} model={model} onSelect={onSelect} />)}
      </div>
    </aside>
  )
}
