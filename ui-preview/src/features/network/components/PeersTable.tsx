      ...
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-2 lg:contents">
        <div className="flex min-w-0 items-center gap-2 lg:contents">
          <div className="flex min-w-0 items-center gap-2">
            <span className="shrink-0 font-mono text-[length:var(--density-type-control)]">
              {peer.shortId ?? peer.id}
            </span>
          ...
          <div className="hidden font-mono text-[length:var(--density-type-caption-lg)] text-fg-dim lg:block">
            {peer.version ?? '—'}
          </div>
        ...
        <div className="col-span-2 grid min-w-0 grid-cols-[auto_auto_auto_minmax(92px,1fr)] items-center gap-x-3 gap-y-1 lg:hidden">
          <div className="font-mono text-[length:var(--density-type-caption-lg)] text-fg-dim">
            {formatLatency(peer.latencyMs)} ms
          </div>
          <div className="font-mono text-[length:var(--density-type-caption-lg)]">
          ...
          <div className="flex items-baseline justify-end gap-1 font-mono text-[length:var(--density-type-caption-lg)] text-fg-dim">
            <span className="text-[length:var(--density-type-annotation)] uppercase tracking-[0.04em] text-fg-faint">
              Tok/s
            </span>
    ...
    <section className="panel-shell min-w-0 overflow-hidden rounded-[var(--radius-lg)] border border-border bg-panel [contain:inline-size]">
      ...
      <div className="max-w-full overflow-x-auto [contain:inline-size]">
        ...
        >
          ...
          <div>VRAM</div>
          <div>Share</div>
          <div className="text-right">Tok/s</div>
        </div>
        {peers.map((peer, i) => (
          <PeerRow
            key={peer.id}
