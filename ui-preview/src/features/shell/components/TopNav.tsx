    ...
    <>
      {harnessMode ? (
        <CopyInstructionRow
          ...
          hint={
            <span>
              App pages are using local fixture data. Switch Data source to Live API in Tweaks to fetch from the
              backend.
            </span>
          }
        />
    ...
    <HeaderHoverCard
      trigger={(triggerProps) => (
        ...
        >
          <span
            className={cn(
              'size-[var(--nav-chip-dot-size)] shrink-0 rounded-full',
              harnessMode ? 'bg-warn' : 'bg-fg-faint'
            )}
            aria-hidden="true"
          />
