...
async function enableConfigurationPage(page: Page) {
  await page.addInitScript(
    ...
    { key: FEATURE_FLAG_OVERRIDES_STORAGE_KEY, value: ENABLE_CONFIGURATION_PAGE_OVERRIDE },
  )
}
...
test('app smoke navigation hides gated configuration by default', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Primary' }).getByRole('link', { name: 'Configuration', exact: true })).toBeHidden()
...
test('configuration blocks in-app navigation when unsaved', async ({ page }) => {
  ...
  await page.getByRole('navigation', { name: 'Primary' }).getByRole('link', { name: 'Configuration', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Configuration', exact: true })).toBeVisible()
  await page.getByRole('tab', { name: 'Model Deployment' }).click()

  const carrackSection = page.locator('section').filter({ has: page.getByRole('button', { name: 'Add model to carrack' }) })
