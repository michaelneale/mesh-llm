...
maybe_build() {
  ...
  run_logged "build" "" \
    env SKIPPY_LLAMA_BUILD_DIR="${SKIPPY_LLAMA_BUILD_DIR:-$ROOT/.deps/skippy-llama.cpp/build-stage-abi-static}" \
...
if [[ -n "$DRAFT_MODEL_PATH" ]]; then
  ...
  spec_args=(
    env
    "SKIPPY_LLAMA_BUILD_DIR=${SKIPPY_LLAMA_BUILD_DIR:-$ROOT/.deps/skippy-llama.cpp/build-stage-abi-static}"
