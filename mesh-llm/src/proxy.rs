...
fn rewrite_http_json_model(request: &BufferedHttpRequest, model_override: &str) -> Option<Vec<u8>> {
    let raw_request = std::str::from_utf8(&request.raw).ok()?;
    let (head, _) = raw_request.split_once("\r\n\r\n")?;
    let mut json = request.body_json.clone()?;
    ...
    match target {
        election::InferenceTarget::Local(port) | election::InferenceTarget::MoeLocal(port) => {
            if let Some(model_override) = launch::mlx_model_override_for_port(port) {
                let forwarded = rewrite_http_json_model(request, &model_override)
                    .unwrap_or_else(|| request.raw.clone());
                return match TcpStream::connect(format!("127.0.0.1:{port}")).await {
                    Ok(mut upstream) => {
                        let _inflight = node.begin_inflight_request();
                        let _ = upstream.set_nodelay(true);
                        if let Err(e) = upstream.write_all(&forwarded).await {
                            tracing::warn!("API proxy: failed to forward buffered request to local mlx backend on {port}: {e}");
                            let _ = send_503(tcp_stream).await;
                            return false;
                        }
            ...
            match TcpStream::connect(format!("127.0.0.1:{port}")).await {
                Ok(mut upstream) => {
                    let _inflight = node.begin_inflight_request();
                    let _ = upstream.set_nodelay(true);
                    if let Err(e) = upstream.write_all(&request.raw).await {
