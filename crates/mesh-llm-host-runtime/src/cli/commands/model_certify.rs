...
/// All CLI arguments for `models certify`, bundled to avoid too-many-arguments.
...
pub(crate) async fn dispatch_model_certify(args: ModelCertifyArgs<'_>) -> Result<()> {
    ...
    let source_ref = source_repo.context(
        "Source repo is required for certification job submission.\n\
         Usage: mesh-llm models certify <source_repo>:<quant> --family <family>",
    )?;
        ...
        (Some(selector), Some(quant)) if selector != quant => {
            bail!(
                "source ref selector '{selector}' conflicts with --quant '{quant}'. \
                 Use `mesh-llm models certify {source_repo}:{selector} --family ...`."
            );
        }
