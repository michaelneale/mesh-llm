...
mod tests {
    ...
    #[test]
    fn config_sync_raw_toml_roundtrips_additive_defaults_sections() {
...
"#,
        ...
        let snapshot = mesh_config_to_proto(&config);
        assert!(
            snapshot.raw_toml.contains("[defaults.model_fit]"),
            "raw TOML should carry additive defaults sections"
        );
