...
import { loadChatState, saveChatState, trimThreadMessages, type ChatState } from './chat-storage'
...
describe('useConversations', () => {
  ...
  it('restores the persisted live-scoped selection with its thread on hydration', async () => {
    const persistedState: ChatState = {
      conversations: [
        ...
        { id: 'live-b', title: 'Live selected', subtitle: '', updatedAt: 'Later' }
      ],
      conversationGroups: [
  ...
  it('normalizes restored conversation groups with a rolling 24-hour Earlier cutoff', async () => {
    ...
    vi.mocked(loadChatState).mockResolvedValue({
      conversations: [
        { id: 'now', title: 'Now', subtitle: '', updatedAt: new Date(now).toISOString() },
        { id: 'still-today', title: 'Still today', subtitle: '', updatedAt: stillToday },
        { id: 'earlier', title: 'Earlier', subtitle: '', updatedAt: earlier }
      ],
      conversationGroups: [
