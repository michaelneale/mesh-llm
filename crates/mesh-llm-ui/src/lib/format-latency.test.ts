...
describe('formatPeerLatency', () => {
  ...
  it('returns <1 for sub-millisecond values', () => {
    expect(formatPeerLatency(0.4)).toBe('<1')
    expect(formatPeerLatency(0.9)).toBe('<1')
  })

  it('rounds to nearest millisecond for normal values', () => {
    expect(formatPeerLatency(1.2)).toBe('1')
    expect(formatPeerLatency(3.4)).toBe('3')
    expect(formatPeerLatency(7.8)).toBe('8')
  })

  it('clamps negative values to zero', () => {
    expect(formatPeerLatency(-5)).toBe('0')
  })
})
...
describe('peerLatencyHint', () => {
  it('returns unknown when latency is null', () => {
    expect(
      peerLatencyHint({ latencyMs: null, source: LatencySource.UNSPECIFIED, ageMs: null, observerId: null })
    ).toBe('unknown')
  })

  it('returns direct for DIRECT source without age suffix when age is zero', () => {
    expect(
      peerLatencyHint({ latencyMs: 5, source: LatencySource.DIRECT, ageMs: 0, observerId: null })
    ).toBe('direct')
  })

  it('returns direct with age suffix for DIRECT source when age is positive', () => {
    expect(
      peerLatencyHint({ latencyMs: 5, source: LatencySource.DIRECT, ageMs: 1200, observerId: 'observer-1' })
    ).toBe('direct (1s old)')
  })

  it('returns estimated for ESTIMATED source', () => {
    expect(
      peerLatencyHint({ latencyMs: 5, source: LatencySource.ESTIMATED, ageMs: null, observerId: null })
    ).toBe('estimated')
  })

  it('returns unknown for UNKNOWN source', () => {
    expect(
      peerLatencyHint({ latencyMs: 5, source: LatencySource.UNKNOWN, ageMs: null, observerId: null })
    ).toBe('unknown')
  })

  it('returns unspecified for UNSPECIFIED source', () => {
    expect(
      peerLatencyHint({ latencyMs: 5, source: LatencySource.UNSPECIFIED, ageMs: null, observerId: null })
    ).toBe('unspecified')
  })

  it('includes age suffix with rounding for ESTIMATED source', () => {
    expect(
      peerLatencyHint({ latencyMs: 5, source: LatencySource.ESTIMATED, ageMs: 2400, observerId: null })
    ).toBe('estimated (2s old)')
  })
})
