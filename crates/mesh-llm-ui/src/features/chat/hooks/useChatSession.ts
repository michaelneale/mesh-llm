...
import { localRoutableModels, peerRoutableModels } from '@/features/app-shell/lib/status-helpers'
import type { TopSection } from '@/features/app-shell/lib/routes'
import type { MeshModel, StatusPayload } from '@/features/app-shell/hooks/useStatusStream'
import { parseDataUrl } from '@/features/chat/lib/chat-attachments'
import { createChatId } from '@/features/chat/lib/chat-id'
import { attachmentForMessage, buildAttachmentBlocks, buildResponsesInput } from '@/features/chat/lib/message-content'
import {
  createConversation,
  createInitialChatState,
  findLastUserMessageIndex,
  loadPersistedChatState as loadPersistedChatStateFromStorage
} from '@/features/chat/lib/chat-storage'
import type {
...
function sanitizeAttachment(raw: unknown): ChatAttachment | null {
  ...
  return {
    ...
    status:
      item.status === 'pending' || item.status === 'uploading' || item.status === 'failed' ? item.status : undefined,
    error: typeof item.error === 'string' ? item.error : undefined,
    extractedText: typeof item.extractedText === 'string' ? item.extractedText : undefined,
    extractionSummary: typeof item.extractionSummary === 'string' ? item.extractionSummary : undefined,
    renderedPageImages:
      Array.isArray(item.renderedPageImages) && item.renderedPageImages.every((value) => typeof value === 'string')
        ? (item.renderedPageImages as string[])
        : undefined,
    imageDescription: typeof item.imageDescription === 'string' ? item.imageDescription : undefined
  }
}
...
function sanitizeMessages(raw: unknown): ChatMessage[] {
  ...
  const sanitized = raw.flatMap((item) => {
    ...
    const attachments = Array.isArray((item as { attachments?: unknown }).attachments)
      ? ((item as { attachments: unknown[] }).attachments.map(sanitizeAttachment).filter(Boolean) as ChatAttachment[])
      : []
    const legacyImage =
      typeof (item as { image?: unknown }).image === 'string' ? (item as { image: string }).image : undefined
    const legacyAudio =
      ...
      typeof (item as { audio: { mimeType?: unknown } }).audio.mimeType === 'string'
        ? {
            ...
            fileName:
              typeof (item as { audio: { fileName?: unknown } }).audio.fileName === 'string'
    ...
    return [
      {
        id: typeof (item as { id?: unknown }).id === 'string' ? (item as { id: string }).id : createChatId(),
        role: safeRole,
        ...
        model: clampText(
          typeof (item as { model?: unknown }).model === 'string' ? (item as { model: string }).model : undefined,
          256
        ),
        stats: clampText(
          typeof (item as { stats?: unknown }).stats === 'string' ? (item as { stats: string }).stats : undefined,
          256
        ),
      ...
      return [
        {
          ...
          updatedAt: typeof obj.updatedAt === 'number' ? String(obj.updatedAt) : String(now),
...
async function writeChatStateToDb(state: ChatState): Promise<void> {
  ...
  try {
    ...
    const store = tx.objectStore(CHAT_DB_STORE)
    store.put({ id: CHAT_DB_KEY, state, updatedAt: String(Date.now()) })
    await transactionToPromise(tx)
  } catch (err) {
...
async function loadPersistedChatState(): Promise<ChatState> {
  const result = await loadPersistedChatStateFromStorage(readChatStateFromDb)
  if (!result) {
...
export function useChatSession({
  ...
  const warmModels = useMemo(() => {
    const models = new Set(meshModels.filter((model) => model.status === 'warm').map((model) => model.name))
  ...
  const attachmentPreparationMessage = useMemo(() => {
    const preparingAttachment = pendingAttachments.find((attachment) => attachment.status === 'uploading')
    ...
    if (isPdfMimeType(preparingAttachment.mimeType) || preparingAttachment.fileName?.toLowerCase().endsWith('.pdf')) {
  ...
  useEffect(() => {
    ...
    if (selectedModel && selectedModel !== 'auto' && warmModels.includes(selectedModel)) return
    // eslint-disable-next-line react-hooks/set-state-in-effect -- syncs selected model with external model availability changes
  ...
  useEffect(() => {
    ...
    if (routedChatId) {
      if (conversations.some((conversation) => conversation.id === routedChatId)) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- syncs chat state with URL routing changes
        setChatState((prev) =>
          prev.activeConversationId === routedChatId ? prev : { ...prev, activeConversationId: routedChatId }
        )
      } else {
  ...
  }, [activeConversation?.id, chatStateHydrated, conversations, replaceChatRoute, routedChatId, section])
  ...
  const canRegenerate =
    canChat && !!activeConversation && findLastUserMessageIndex(activeConversation.messages ?? []) >= 0

  const updateChatState = useCallback((updater: (prev: ChatState) => ChatState) => {
    setChatState((prev) => updater(prev))
  }, [])

  const markComposerAttachment = useCallback((attachmentId: string, patch: AttachmentStatePatch) => {
    ...
    async (params: {
      ...
      try {
        ...
        if (!response?.ok || !response?.body) {
          const errorMessage = response ? await parseApiErrorBody(response) : 'HTTP unknown'
          ...
          while (frameEnd >= 0) {
            ...
            try {
              ...
              if (eventName === 'response.output_text.delta') {
                const contentDelta = typeof payload.delta === 'string' ? payload.delta : ''
                if (!contentDelta) {
                  frameEnd = buffer.indexOf('\n\n')
                  continue
                }
                ...
                const responsePayload =
                  payload.response && typeof payload.response === 'object'
                    ...
                    : null
                if (responsePayload && typeof responsePayload.output_text === 'string' && !full) {
                  full = responsePayload.output_text
                }
                if (responsePayload && responsePayload.usage && Number.isFinite(responsePayload.usage.output_tokens)) {
        ...
        if (err instanceof Error && err.name === 'AbortError') {
          ...
          updateChatState((prev) => ({
            ...
            conversations: updateConversationList(prev.conversations, conversationId, (conversation) => ({
              ...conversation,
              messages: (conversation.messages ?? []).map((message) =>
                message.id === assistantId ? { ...message, content: `Error: ${errorMessage}`, error: true } : message
              ),
              updatedAt: String(Date.now())
            }))
          }))
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      const userMessage: ChatMessage = {
        ...
        model,
        attachments:
          normalizedPendingAttachments.length > 0 ? normalizedPendingAttachments.map(attachmentForMessage) : undefined
      }
      ...
      const nextTitle =
        (activeConversation?.title === DEFAULT_CHAT_TITLE && existingMessages.length === 0) || !activeConversation
      ...
      updateChatState((prev) => ({
        ...(prev.conversations.some((conversation) => conversation.id === conversationId)
          ...
          : {
              ...
              conversationGroups: prev.conversationGroups,
  ...
  const deleteConversation = useCallback(
    (conversationId: string) => {
      ...
      const nextActiveId =
        activeConversationId === conversationId ? (remaining[0]?.id ?? null) : activeConversationId || null
      updateChatState((prev) => ({
        ...prev,
        conversations: prev.conversations.filter((conversation) => conversation.id !== conversationId),
        activeConversationId:
          prev.activeConversationId === conversationId ? (nextActiveId ?? '') : prev.activeConversationId
      }))
  ...
  const clearAllConversations = useCallback(() => {
    updateChatState(() => ({ conversations: [], conversationGroups: [], threads: {}, activeConversationId: '' }))
