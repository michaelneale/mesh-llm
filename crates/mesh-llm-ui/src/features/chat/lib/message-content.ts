...
import { messageAttachments, parseDataUrl } from '@/features/chat/lib/chat-attachments'
import type { AttachmentStatePatch, ChatAttachment, ChatMessage } from '@/features/chat/lib/chat-types'
...
export async function buildAttachmentBlocks(
  ...
  for (const sourceAttachment of attachments) {
    ...
    if (extractedText) {
      const label = attachment.fileName ? `[Content from ${attachment.fileName}]` : '[Extracted PDF content]'
      contentBlocks.push({
        type: 'input_text',
        text: `${label}\n\n${extractedText}`
      })
...
export async function buildResponsesInput(
  ...
  return Promise.all(
    historyForRequest.map(async (message) => {
      const contentBlocks: Array<Record<string, unknown>> = prebuiltContentByMessageId?.[message.id]?.slice() ?? []
      ...
      if (!prebuiltContentByMessageId?.[message.id] && attachments.length > 0) {
        contentBlocks.push(...(await buildAttachmentBlocks(attachments, requestId, clientId)))
      }
      return {
        role: message.role,
        content:
          contentBlocks.length === 1 && attachments.length === 0 && contentBlocks[0].type === 'input_text'
            ? message.content
            : contentBlocks
      }
    })
