...
export function narrowPairDiffToChangedComponent({
  ...
  const outgoingPairKeys = new Set([...previous].filter((pairKey) => !current.has(pairKey)))
  const incomingPairKeys = new Set([...current].filter((pairKey) => !previous.has(pairKey)))
  ...
  if (addedNodeIdSet.size > 0) {
    ...
    for (const pairKey of changedPairKeys) {
      const pairNodeIds = collectPairNodeIds(pairKey, previousPairRouteSignatures, currentPairRouteSignatures)
    ...
    return {
      outgoingPairKeys: new Set(
        [...outgoingPairKeys].filter((pairKey) => {
          const endpoints = parsePairKey(pairKey)
          return endpoints ? localNodeIds.has(endpoints[0]) && localNodeIds.has(endpoints[1]) : false
        })
      ),
