...
} from '@/features/dashboard/components/topology/layout/distribution'
import { clamp, mixColor, withAlpha } from '@/features/dashboard/components/topology/helpers'
import type {
  ColorTuple,
  LineBuilderInput,
  LineBuilderOutput,
...
import { LIGHT_GRAPHITE, LIGHT_INK, LIGHT_SLATE } from '@/features/dashboard/components/topology/theme/color-tokens'
...
function cachedBuildTopologyPairPlan(screenNodes: ScreenNode[], centerNodeId?: string): TopologyGraphEdge[] {
  let hash = 0x811c9dc5
...
function pushLineSegment(
  ...
  linePositions.push(fromX * devicePixelRatio, fromY * devicePixelRatio, toX * devicePixelRatio, toY * devicePixelRatio)
  lineColors.push(
    fromColor[0],
    fromColor[1],
    fromColor[2],
...
function compactRoutePoints(points: Point[]) {
  ...
  for (const point of points) {
    ...
    if (previous && Math.hypot(point.x - previous.x, point.y - previous.y) <= ROUTE_POINT_EPSILON_PX) {
      continue
    }
...
function buildRouteSignature(mode: 'straight' | 'relay', options: { blockerIds?: string[] } = {}) {
...
function buildConnectionPoint(
  ...
  const inset = Math.min(centerDistance * 0.3, 6 + node.size * (isClientNode(node) ? 0.5 : 0.38) + boundaryBoost * 8)
...
function buildRoutedLine(from: ScreenNode, to: ScreenNode, screenNodes: ScreenNode[], depth = 0): LineRoute | null {
...
export function buildProximityLines({
  ...
  const pushLine = (left: ScreenNode, right: ScreenNode, distance: number, threshold: number, pairKey: string) => {
    ...
    const touchesHighlight = highlightedNodeIds.has(left.id) || highlightedNodeIds.has(right.id)
    ...
    const neutralMix = (touchesHighlight ? 0.42 : touchesCenter ? 0.36 : 0.33) + boundaryBoost * 0.08
    const focusMix =
      (touchesHighlight ? 0.25 : touchesCenter ? 0.22 : 0.19) + boundaryBoost * 0.05 - (touchesClient ? 0.05 : 0)
    const targetInk = touchesClient ? LIGHT_SLATE : LIGHT_INK
    const leftInk = mixColor(mixColor(left.lineColor, LIGHT_GRAPHITE, neutralMix), targetInk, focusMix)
    const rightInk = mixColor(mixColor(right.lineColor, LIGHT_GRAPHITE, neutralMix), targetInk, focusMix)
