...
function flattenPlacements(bands: Array<Array<TopologyAngularPlacement<TestNode>>>, selfNode: TopologyGraphNode) {
  return [
...
describe('distribution crossing helpers', () => {
  ...
  it('deterministically swaps angular slots when that removes a bad crossing', () => {
    ...
    const optimizedBands = optimizeTopologyPlacementForPlan([lockedBand, movableBand], selfNode, crossingPlan)
...
describe('buildTopologyPairPlan', () => {
  it('keeps the cheapest relay chain when a node is first discovered by a more expensive root edge', () => {
    ...
    expect(pairKeys(plan)).toEqual(['n1::root', 'n1::n2', 'n2::n3', 'n3::n4', 'n4::n5'].sort())
  })
