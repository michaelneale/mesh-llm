...
import type { RenderNode, RenderVariant } from '@/features/dashboard/components/topology/types'
import {
...
import type { TopologyAngularPlacement } from '@/features/dashboard/components/topology/layout/distribution'
...
export function useRadarFieldNodes(
  ...
  const renderNodes = useMemo<RenderNode[]>(() => {
    ...
    const selectedServing = others.filter(
      (node) => !node.client && !!focusModel && node.servingModels.some((model) => model === focusModel)
    )
    const selectedIds = new Set(selectedServing.map((node) => node.id))
    const serving = others.filter(
      (node) =>
        !node.client && !selectedIds.has(node.id) && node.servingModels.some((model) => model && model !== '(idle)')
    )
    ...
    const minLatency = latencyNodes.length > 0 ? Math.min(...latencyNodes.map((node) => Number(node.latencyMs))) : 0
    const maxLatency = latencyNodes.length > 0 ? Math.max(...latencyNodes.map((node) => Number(node.latencyMs))) : 1
    ...
    const selfSelectedModelMatch = !!focusModel && selfNode.servingModels.some((model) => model === focusModel)
    ...
    const reusePreviousPlacement =
      shouldStabilizeExistingNodes || shouldStabilizeForRemoval
    ...
    const [optimizedClientNodes, optimizedWorkerNodes, optimizedActiveNodes, optimizedServingNodes] =
      shouldStabilizeForRemoval ? preparedBands : reduceTopologyCrossings(preparedBands, selfLayoutNode, selfNode.id)
  ...
  }, [fallbackModel, nodes, renderVariant, selectedModel, previousLayout, previousSignatures, previousContext])
