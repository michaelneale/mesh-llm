...
describe('WakeableCapacity', () => {
  ...
  it('hides the section when wakeable_nodes is an empty array', () => {
    renderDashboard(buildStatus({ wakeable_nodes: [] }), { wakeableCapacityEnabled: true })
  ...
  it('hides the section when wakeable_nodes is absent', () => {
    renderDashboard(buildStatus(), { wakeableCapacityEnabled: true })

    expect(screen.queryByTestId('wakeable-capacity-section')).not.toBeInTheDocument()
  })

  it('hides the section when the feature flag is disabled', () => {
    renderDashboard(
      buildStatus({
        wakeable_nodes: [
          {
            logical_id: 'test-node-1',
            state: 'sleeping',
            models: ['Qwen2.5-72B-Instruct'],
            vram_gb: 80,
            provider: 'Vast'
...
function renderDashboard(status: StatusPayload, options: { wakeableCapacityEnabled?: boolean } = {}) {
  if (options.wakeableCapacityEnabled != null && options.wakeableCapacityEnabled) {
    window.localStorage.setItem(
      ...
      JSON.stringify({ dashboard: { wakeableCapacity: true } })
    )
  }
