...
type DetailPanelEntry = { kind: 'node'; nodeId: string } | { kind: 'model'; modelName: string }
...
export function useDashboardDetailStack({
  ...
  const activeDetail = useMemo(() => detailPanelStack[detailPanelStack.length - 1] ?? null, [detailPanelStack])
