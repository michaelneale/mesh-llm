...
const API_ORIGIN = 'http://127.0.0.1:9337'
...
function resolveAppUrl(pathname: string, testInfo: TestInfo) {
  ...
  const port = env.PLAYWRIGHT_PORT ?? '51973'
...
test('live shell metadata shows status-backed API and invite instructions', async ({ page }, testInfo) => {
  ...
  await expect(apiCard.getByText('http://mesh.local:3131/v1', { exact: true })).toBeVisible()
  await expect(apiCard.getByText('curl http://mesh.local:3131/v1/models', { exact: true })).toBeVisible()
  await expect(apiCard.getByText('OPENAI_BASE_URL=http://mesh.local:3131/v1', { exact: true })).toBeVisible()
  ...
  await expect(joinCard.getByText('invite-token-123', { exact: true })).toBeVisible()
  await expect(joinCard.getByText('mesh-llm --auto --join invite-token-123', { exact: true })).toBeVisible()
  await expect(joinCard.getByText('mesh-llm client --join invite-token-123', { exact: true })).toBeVisible()
...
test('streaming failure leaves partial text and recoverable chat state', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('alert')).toContainText('Streaming request failed after partial text')
  await expect(page.getByRole('button', { name: 'Send' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Retry last' })).toBeEnabled()
...
test('live chat uploads an attachment before sending the parity request', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: 'Chat' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Send' })).toBeVisible()
  await expect(page.getByText('mistral-small', { exact: true })).toBeVisible()
