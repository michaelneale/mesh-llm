...
describe('ChatPage', () => {
  ...
  it('opens, saves, prefills, and sends the chat-wide system prompt for new chats', async () => {
    ...
    await user.type(within(dialog).getByLabelText('System prompt'), 'Answer as a mesh-llm operator.')
  ...
  it('treats whitespace-only system prompts as cleared state', async () => {
    ...
    await user.type(within(dialog).getByLabelText('System prompt'), '   {Enter}  ')
    await user.click(screen.getByRole('button', { name: 'Save prompt' }))

    await waitFor(() => {
      expect(window.localStorage.getItem(APP_STORAGE_KEYS.chatSystemPrompt)).toBeNull()
      expect(systemPromptButton).toHaveFocus()
    })
