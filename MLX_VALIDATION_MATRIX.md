...
| Phi-3 | mini 4k instruct | `/Users/jdumay/code/worktrees/mesh-llm-validation/output/phi3/Phi-3-mini-4k-instruct-Q8_0.gguf` | `/Users/jdumay/code/worktrees/mesh-llm-validation/mlx/phi3-mini-4k-instruct-4bit` | `HIGH` | 2026-04-07 | `PASS` | `FAIL` | `PENDING` | `PENDING` | `MLX WORSE` | `PARTIAL` | Same-origin local parity pair derived from `microsoft/Phi-3-mini-4k-instruct`. This branch now has real Phi-3 MLX support: the model loads, serves, and matches GGUF semantically across the exact suite. The remaining strict exact failures are formatting-only on the MLX side: `primary -> Blue` and `alt-red -> Red`, while GGUF returns lowercase `blue` and `red`. Keep this row as a live polish target rather than a loader-blocked failure. |
